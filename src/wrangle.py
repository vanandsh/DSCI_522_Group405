...
def main(source_file, target_location):
  ...
  train_file = target_location + "/train.csv"
  test_file = target_location + "/test.csv"

  full_train.to_csv(train_file,header=True, index=False)
  full_test.to_csv(test_file)

def are_files_created(file_directory):
    train_file = file_directory + "/train.csv"
    test_file = file_directory + "/test.csv"
